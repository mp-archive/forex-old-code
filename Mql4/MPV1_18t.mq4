...
int start()
  {   if (xe==true) return(0);
      if (set_hours) b_reverse=set_hour[Hour()];
      if (TimeCurrent()-lastcheck>freq_check_sec) F_check_open2();
      if (TimeCurrent()-lastcalculation>freq_calc_sec && (Hour()==hour_in || hour_in==-1) && b_reverse!=0)
         {  F_calculate1();
