...
void OnTick()
  { if (IsTesting() && testfreq>1) testfrcnt=(testfrcnt+1)%testfreq;
         displfrcn=(displfrcn+1)%5;  if (IsTesting()==false && displfrcn==0) 
         F_displaysimple();  
   F_SetMQL4PreDefVar(strat_symbol);  
   F_skip(1);     
...
int F_signal(int entry_ty2)
   {     int temp_signal;
         ...
         if (allowdoublecurr==false) if (F_currallowed(strat_symbol,temp_signal)==false) return(0);
...
void F_check_open(bool init,bool forced)
   {  int j=0;
     ...
     */
      ...
      if (PositionSelect(F_Curr(strat_symbol_m[j],Symbol())==true)) if (MAGIC_Rf_m[j]==OrderMagicNumber())  F_setup_multistrat(j);
...
void F_open_now(int order_type,int order_type2)
   {  if (order_type==0) {return;}
      ...
      if ((order_type2==1 && order_type==-1) || order_type2==4) {  
            open_array_ticket[open_nr+1]=OrderSend2(strat_symbol,OP_SELL,lots2,base_price,3,sl_price2,tp_price2,program_name+strat_symbol,MAGIC_Rf,0,Green);   
            open_nr++; lastopen=TimeCurrent(); lastopen2=TimeCurrent();lastclose=TimeCurrent()-24*60*60;  
            profitmaxpip=0.0;profitminpip=0.0;profitactpip=0.0; profitfallback=0.0; tp_avg2=tp_avg; open_bool=true; initialequity=(AccountEquity()-hidden_equity);
...
void F_displaysimple()
   ...
   Comment(StringFormat("Show prices\nOpen = %G\nOpenNr = %G\nSymbol = %G\nMagic = %G\ntp =  %G\nsl = %d",open_bool,open_nr,strat_symbol,DoubleToString(MAGIC_Rf,0),tp_price,sl_price));
   }
