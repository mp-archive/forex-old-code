...
bool F_currallowed(string curr_in,int order_in)
   {
    int i3,j3;
    string curr_act;
    curr_allowed=true;   
    ...
    for (i3=1;i3<=curr_nr;i3++) if ((curr_exposure[i3][i3]>0 && curr_order[i3][i3]>0) || (curr_exposure[i3][i3]<0 && curr_order[i3][i3]<0)) curr_allowed=false;
    return(curr_allowed);            
   }
...
int F_signal(int entry_ty2)
   {     int temp_signal;
         ...
         //- van signal -//
           ...
           if (PositionsTotal()>0) if (allowdoublecurr==false) if (F_currallowed(strat_symbol,temp_signal)==false) return(0);
...
void F_close(int order_type)
   {  bool b_lezar=false; int b_lezar_kod=0;         
      ...
      if (order_type==1 && Bid<=sl_price && sl_price!=0) { b_lezar=true; b_lezar_kod=12;} 
      if (order_type==1 && Ask>=tp_price && tp_price!=0) { b_lezar=true; b_lezar_kod=13;}       
      if (order_type==-1 && Ask>=sl_price && sl_price!=0) { b_lezar=true; b_lezar_kod=14;}       
      if (order_type==-1 && Bid<=tp_price && tp_price!=0) { b_lezar=true; b_lezar_kod=15;} 
...
void F_displaysimple()
   ...
   if (PositionSelect(strat_symbol)==true && curr_exposureabs==0) F_currallowed(strat_symbol,0);
