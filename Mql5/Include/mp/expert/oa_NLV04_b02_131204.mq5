   ...
   input double in_tp2=1.0;
   input double in_sl2=1.0;
    double     in_shl=10.0,in_shm=3.0; 
   /**/   bool     set_hours=false;
         bool     set_days=false;
        
          
   input string mpversion=" --- ver 131003 0939 --- ";
   
//---------------- CORE_SYSTEM -------------------------------
//------------------------------------------------------------
   //---------------- Testing -----------------------------------

   input int        in_testfreq=1;  int testfrcnt=0; int displfrcn=0;  // to make test faster
      enum teststattype {ULTREC=1,LASTPROFP=2,RDD=3,WITHDRAW=4,NRLOSS=5,DDABS=6,RECOV2=7};
   input teststattype in_owntesterstat=2;
         teststattype owntesterstat=in_owntesterstat;

   
   
   
          input double testerwd_over=0;
          double     tester_ts=0,tester_tp=0,tester_tp2;
          datetime   tester_lastwd;
...
double lots=in_lots;
double lots_multipliyer=in_lots_multipliyer*in_lots_adjust;
...
int closefridaynight= in_closefridaynight;
int closesar=in_closesar;
...
int testfreq=in_testfreq;
 ...
 double tp_2=in_tp2;
    ...
    bool monday=in_monday;
    bool tuesday=in_tuesday;
    bool wednesday=in_wednesday;
    bool thursday=in_thursday;
    bool friday=in_friday;
    bool saturday=in_saturday; 
...
int OnInit()
  {  F_SetMQL4PreDefVar(strat_symbol);
      ...
      for (int i=0;i<=strat_nr;i++) MAGIC_Rf_m[i]=StringToInteger(in_MAGIC_Rf)+i+1;            
      if (test_strategies_to>0) for (int i=0;i<=strat_nr;i++) { if(i==test_strategies_to-1) strat_run[i]=true; else strat_run[i]=false;}
      if (test_strategies_to<0) for (int i=0;i<=strat_nr;i++) { if(i==-test_strategies_to-1) strat_run[i]=false; else strat_run[i]=true;}

      lastopen=TimeCurrent()-5*60;
      //tester_lastwd=TimeCurrent();
...
void OnTick()
  { if (IsTesting() && testfreq>1) testfrcnt=(testfrcnt+1)%testfreq;
         displfrcn=(displfrcn+1)%2;  if (IsTesting()==false && displfrcn==0) 
         F_displaysimple();  
    //if (IsTesting() && testerwd_over>0) if (AccountEquity()>testerwd_over*2 && TimeCurrent()-tester_lastwd>2*24*60*60)  { tester_tp2+=AccountEquity()-testerwd_over ; TesterWithdrawal(AccountEquity()-testerwd_over);tester_lastwd=TimeCurrent();}
    if (IsTesting() && testerwd_over>0) if (AccountEquity()>testerwd_over && TimeCurrent()-tester_lastwd>30*24*60*60)  { tester_tp2+=AccountEquity()-testerwd_over ; TesterWithdrawal(AccountEquity()-testerwd_over);tester_lastwd=TimeCurrent();}
    //if (IsTesting() && owntesterstat==4) if (AccountEquity()>5000 && Day()==1 && TimeCurrent()-tester_lastwd>2*24*60*60) { tester_tp2+=AccountEquity()-5000 ; TesterWithdrawal(AccountEquity()-5000);tester_lastwd=TimeCurrent();}
   F_SetMQL4PreDefVar(strat_symbol);  
   F_skip(1);     
   if (IsTesting() || (in_b_run_multistrat==true && MAGIC_Rf==in_MAGIC_Rf) || MAGIC_Rf!=OrderMagicNumber()) F_check_open(true,true); F_check_open(false,false);
   ...
   if (open_bool==false && b_run_multistrat==false && testfrcnt==0) 
      {  signal1=F_signal(entry_type);  
         ...
         F_SetMQL4PreDefVar(strat_symbol); F_skip(1); F_check_open(false,true);F_set_tp_sl(); // if (in_hidden_sl==true) F_change(sl_price2,tp_price2);// else F_change(sl_price,tp_price);
         Print(tp_price,"    ", tp_price2,"  " ,open_type, "   tp:   ",tp,"   ",tp_2,"  ",Point, "   ",  sl_price, "   " , sl_price2);
         if (OrderLots()>(AccountBalance()-hidden_equity)/in_maxequityperlot) OrderClose3(strat_symbol,NormalizeDouble(OrderLots()-(AccountBalance()-hidden_equity)/in_maxequityperlot,2),"Close/Too big lot  "+MAGIC_Rf); 
         }}
...
//ULTREC=1,LASTPROFP=2,RDD=3,WITHDRAW=4,NRLOSS=5,DDABS=6,RECOV2=7
 double OnTester()
{  double res=0.0,res1=0.0,res2=0.0,res3=0.0,res4=0.0,res5=0.0,res6=0.0,res7=0.0,tester_tmp=0.0;
   for (int it=0;it<=20;it++) {if ((tester_typ[it]<tester_tmp || tester_tmp==0) && tester_typ[it]!=0) tester_tmp=tester_typ[it]; if (tester_typ[it]!=0) Print("year ",it,"= ",tester_typ[it]);} 
   
   if (tester_ts!=0) res1=MathRound(TesterStatistics(STAT_PROFIT)/TesterStatistics(STAT_EQUITY_DD)/(tester_ts/(60*60*24*250))*1000)/1000;         
   if (TesterStatistics(STAT_PROFIT)!=0) res2=MathRound(tester_tp/TesterStatistics(STAT_PROFIT)*1000)/1000;
   res3=MathRound(TesterStatistics(STAT_EQUITY_DDREL_PERCENT)*100)/100;
   res4=MathRound(tester_tp2); //+TesterStatistics(STAT_PROFIT);
   res5=TesterStatistics(STAT_LOSS_TRADES);
   res6=TesterStatistics(STAT_EQUITY_DD);  
   if (TesterStatistics(STAT_EQUITY_DDREL_PERCENT)!=0) res7=MathRound((TesterStatistics(STAT_PROFIT)/TesterStatistics(STAT_INITIAL_DEPOSIT)+1)/TesterStatistics(STAT_EQUITY_DDREL_PERCENT)*10000/5)/100;
   Print("ULTREC    ",res1);
   Print("LASTPROFP ",res2);   
   Print("RDD       ",res3);
   Print("WITHDRAW  ",res4);   
   Print("NR LOSS   ",res5);  
   Print("DD max    ",res6);     
   Print("Recovery20  ",res7);        
   
   switch(owntesterstat)
   {
   case 1: {  res=res1;if (TesterStatistics(STAT_TRADES)<20) res=0; }; break;
   case 2: {  res=res2;if (TesterStatistics(STAT_TRADES)<20) res=0; }; break;
   case 3: {  res=res3; }; break;
   case 4: {  res=res4;if (AccountEquity()<300) res=0;  }; break;
   case 5: {  res=res5; }; break;   
   case 6: {  res=res6; }; break;      
   case 7: {  res=res7; }; break;       
   default:          res=res2; break;
   }    
...
int F_currhandling(string curr_in,int order_in)
   {
    int i3,j3;
    string curr_act;
    currhnd=1;   
    ...
    for (i3=1;i3<=curr_nr;i3++) for (j3=1;j3<=curr_nr;j3++) {curr_order[i3][j3]=0;curr_exposure[i3][j3]=0;}
    for (i3=1;i3<=curr_nr;i3++) for (j3=1;j3<=curr_nr;j3++) if (i3<j3)
    { 
      curr_act=curr[i3]+curr[j3];
      if (PositionSelect(curr[i3]+curr[j3])==true) { curr_exposure[i3][j3]+=OrderType2();curr_exposure[i3][i3]+=OrderType2(); curr_exposure[j3][j3]-=OrderType2();curr_exposureabs+=1; }
      if (curr_in==curr_act) {curr_order[i3][j3]+=order_in;curr_order[i3][i3]+=order_in; curr_order[j3][j3]-=order_in; }  
    }
    for (i3=1;i3<=curr_nr;i3++) 
      {  if ((curr_exposure[i3][i3]>0 && curr_order[i3][i3]>0) || (curr_exposure[i3][i3]<0 && curr_order[i3][i3]<0)) currhnd=3;
         if ((curr_exposure[i3][i3]>0 && curr_order[i3][i3]<0) || (curr_exposure[i3][i3]<0 && curr_order[i3][i3]>0)) currhnd=2;
         if (curr_exposure[i3][i3]==0 && curr_order[i3][i3]!=0) currhnd=1; }
    return(currhnd);            
   }
...
int F_signal(int entry_ty2)
   {     int temp_signal;
         ...
         //if (b_compeetingbot==false && PositionSelect(strat_symbol)==true)     return(0);      //  <<----change for parallel run
         //if (PositionSelect(strat_symbol)==true); return(0);  
         if (PositionsTotal()>=in_maxpos || PositionSelect(strat_symbol)==true) return(0);
         ...
         if (monday==false && DayOfWeek()==1) return(0);
         if (tuesday==false && DayOfWeek()==2) return(0);
         if (wednesday==false && DayOfWeek()==3) return(0);
         if (thursday==false && DayOfWeek()==4) return(0);
         if (friday==false && DayOfWeek()==5) return(0);
         if (saturday==false && DayOfWeek()==6) return(0);
         ...
         if (TimeCurrent()-lastloss<in_ult_delayminutesafterloss*60) return(0);         
                  
         temp_signal=F_signal_b(entry_ty2); //if (temp_signal!=0) signal1_timee=TimeCurrent();        
         if (temp_signal==0) return(0);
         //- van signal -//
         
           if (PositionsTotal()>0) if (in_curr_handling<F_currhandling(strat_symbol,temp_signal)) return(0);
           //if (TimeCurrent()-lastopen<=10 || OrdersTotal()>0) return(0);  
           if (TimeCurrent()-GGetLastSendOrder() <=10 || OrdersTotal()>0) return(0);             
...
void F_check_open(bool init,bool forced)
   {  int j=0;
     ...
     /*
      ...
      if (PositionSelect(strat_symbol)==true)
     */
     
     
      // hiányzik ->   több pozíció adatainak eltárolása,   deviza-ciklusban kell lennie a close-nak is nemcsak a checkopen-nak     
      // illetve ha egyszerre csak egy pozi van nyitva akkor mûködhet 
     
      if (b_run_multistrat==true) if (init==true || forced==true)
      for (j=0;j<=strat_nr-1;j++) if (strat_run[j]==true) 
      if (PositionSelect(F_Curr(strat_symbol_m[j],Symbol()))==true && MAGIC_Rf_m[j]==OrderMagicNumber()) 
      {F_setup_multistrat(j); /*Print("strat : ",j,"    ",strat_symbol); */ break;}
         
      //if (PositionSelect(strat_symbol)==true) if (init==true)
      //   for (j=0;j<=strat_nr-1;j++) if (strat_run[j]==true && MAGIC_Rf_m[j]==OrderMagicNumber()) {F_setup_multistrat(j);}
               
      if (PositionSelect(strat_symbol)==true) //if (in_MAGIC_Rf<=OrderMagicNumber() && in_MAGIC_Rf+1000>OrderMagicNumber())
...
void F_set_lots_martingal()
   {  /*
      ...
      */   
      ...
      lots_martingal=lots;
      lots_martingal=MathMin(MathMin(MarketInfoMQL4(strat_symbol,MODE_MAXLOT),lots_martingal),in_maxlot);
...
void F_open_now(int order_type,int order_type2)
   {  if (order_type==0) {return;}     
      ...
      if ((order_type2==1 && order_type==1) || order_type2==4)  {  
            open_array_ticket[open_nr+1]=OrderSend2(strat_symbol,OP_BUY,lots2,base_price,3,sl_price2,tp_price2,MAGIC_Rf + "  tp: " +(tp+tp_2)+ "  sl: " +(sl+sl_2),MAGIC_Rf,0,Green);  
      ...
      if ((order_type2==1 && order_type==-1) || order_type2==4) {  
            open_array_ticket[open_nr+1]=OrderSend2(strat_symbol,OP_SELL,lots2,base_price,3,sl_price2,tp_price2,MAGIC_Rf + "  tp: " +(tp+tp_2)+ "  sl: " +(sl+sl_2),MAGIC_Rf,0,Green);   
            open_nr++; lastopen=TimeCurrent(); lastopen2=TimeCurrent();lastclose=TimeCurrent()-24*60*60;  
            profitmaxpip=0.0;profitminpip=0.0;profitactpip=0.0; profitfallback=0.0; tp_avg2=tp_avg; open_bool=true; initialequity=(AccountEquity()-hidden_equity);
     ...
     // limitorder    order_type2 (entry_ordertype=2)
      if (pending_bool==false) {
         if ((order_type2==2 && order_type==1) || order_type2==5)  {  
            ...
            bestprice=0.0; }
         if ((order_type2==2 && order_type==-1) || order_type2==5)  {  
            pending_array_ticket[pending_nr+1]=OrderSend2(strat_symbol,OP_SELLLIMIT,lots2,base_price+entry_orderbridge*Point,3,0,0,program_name+strat_symbol,MAGIC_Rf,TimeCurrent()+entry_expire*60,Green);    
            pending_nr++; lastopen=TimeCurrent(); lastopen2=TimeCurrent();lastclose=TimeCurrent()-24*60*60;  
            profitmaxpip=0.0;profitminpip=0.0;profitactpip=0.0; profitfallback=0.0; tp_avg2=tp_avg; pending_bool=true; initialequity=(AccountEquity()-hidden_equity);
            bestprice=0.0; }
         }
     // stoporder     order_type2 (entry_ordertype=3)
      if (pending_bool==false) { 
         if ((order_type2==3 && order_type==1) || order_type2==6)  {  
            ...
            bestprice=0.0; }
         if ((order_type2==3 && order_type==-1) || order_type2==6) {  
            pending_array_ticket[pending_nr+1]=OrderSend2(strat_symbol,OP_SELLSTOP,lots2,base_price-entry_orderbridge*Point,3,0,0,program_name+strat_symbol,MAGIC_Rf,TimeCurrent()+entry_expire*60,Green);    
            pending_nr++; lastopen=TimeCurrent(); lastopen2=TimeCurrent();lastclose=TimeCurrent()-24*60*60;  
            profitmaxpip=0.0;profitminpip=0.0;profitactpip=0.0; profitfallback=0.0; tp_avg2=tp_avg; pending_bool=true; initialequity=(AccountEquity()-hidden_equity);
            bestprice=0.0; }
         }            
      ...
      PositionSelect(strat_symbol);
      Print(" VVVVÉTEL!!!!    ",strat_symbol,"  ",MAGIC_Rf,"  ",OrderMagicNumber(),"  ",order_type);   
      return;}
...
void F_closepart(int order_type)
   ...
   if (closeinstepup!=0.0 && closepipstepup!=0.0)
      ...
      if (profitactpip>closepipstepup*Point*closestepup) 
         ...
         OrderClose3(strat_symbol,proportionlotup,"/Close "+MAGIC_Rf+" proportionlotup   ");  }}
   ...
   if (closeinstepdown!=0.0 && closepipstepdown!=0.0)
      ...
      if (profitactpip<-closepipstepdown*Point*closestepdown) 
         ...
         OrderClose3(strat_symbol,proportionlotdown,"/Close "+MAGIC_Rf+" proportionlotdown");  }}
   ...
   if (sl_turn!=0.0 && sl_turnmult!=0.0)
      {
         ...
         if (profitactpip<-sl_turn*Point) 
            ...
            OrderClose3(strat_symbol,volume1,"/Close "+MAGIC_Rf+" turn");  
            F_check_open(false,true);  
...
void F_close(int order_type)
   {  bool b_lezar=false; int b_lezar_kod=0;         
      ...
      if (b_letprofitrun==false && profitactpip>=tp_avg2*Point && MathAbs(profitactpip-tp_avg2)<2000) { b_lezar=true; b_lezar_kod=3;}   
      if (b_letprofitrun==true && profitmaxpip>=tp_avg2*Point  && MathAbs(profitmaxpip-tp_avg2)<2000 && profitfallback>=fallbackpip_min*Point && profitmaxpip>0.0)
      ...
      if (order_type==1 && Bid<=sl_price && sl_price!=0 && Bid!=0 && MathAbs(Bid-sl_price)/Point<2000) { b_lezar=true; b_lezar_kod=12;} 
      if (order_type==1 && Ask>=tp_price && tp_price!=0 && Ask!=0 && MathAbs(Ask-tp_price)/Point<2000) { b_lezar=true; b_lezar_kod=13;}       
      if (order_type==-1 && Ask>=sl_price && sl_price!=0 && Ask!=0 && MathAbs(Ask-sl_price)/Point<2000) { b_lezar=true; b_lezar_kod=14;}       
      if (order_type==-1 && Bid<=tp_price && tp_price!=0 && Bid!=0 && MathAbs(Bid-tp_price)/Point<2000) { b_lezar=true; b_lezar_kod=15;} 
      if (DayOfWeek()>=5 && Hour()>=closefridaynight && closefridaynight!=0) { b_lezar=true; b_lezar_kod=16;} 
      if (closesar==1) {temp_sar=iSARMQL4(strat_symbol,strat_period,0.02,0.2,1);
               if ((order_type==1 && Bid<temp_sar) || (order_type==-1 && Ask>temp_sar)) { b_lezar=true; b_lezar_kod=17;}}
      ...
      if (b_lezar==false) return;
         ...
         if (b_lezar==true && TimeCurrent()-GGetLastSendOrder() >5)    
         {
           lastprofit=0;
           PositionSelect(strat_symbol);
           lastprofit= PositionGetDouble(POSITION_PROFIT)+PositionGetDouble(POSITION_COMMISSION)+PositionGetDouble(POSITION_SWAP);
           while (PositionGetDouble(POSITION_VOLUME)>in_maxlot) {OrderClose3(strat_symbol,in_maxlot,"/Close "+MAGIC_Rf+" bigger than maxlot"); }       
           OrderClose3(strat_symbol,0,"Close "+MAGIC_Rf+" normal");             
           Print("  ELADÁS !!!" , lots2, "      ",lastprofit, "      ",  open_nr, "   kód: ",b_lezar_kod, "  sl/tp:  ",sl_price,"  ",tp_price,"  ","  Ask:",Ask,"   Bid:",Bid,"  ProfPip: ",profitactpip,"  ",MAGIC_Rf);  open_nr=0; 
...
void F_setup_multistrat(int nr)
   ...
   strat_symbol=F_Curr(strat_symbol_m[nr],Symbol());
    if (b_run_multistrat==true)  F_SetMQL4PreDefVar(strat_symbol);
   ...
   sl_onavgprice           =sl_onavgprice_m[nr];               
      ...
      closefridaynight=closefridaynight_m[nr];
      closesar=closesar_m[nr];
   ...
   delayminutesafterloss=delayminutesafterloss_m[nr];
    ...
    monday=monday_m[nr];
    tuesday=tuesday_m[nr];
    wednesday=wednesday_m[nr];
    thursday=thursday_m[nr];
    friday=friday_m[nr];
    saturday=saturday_m[nr];  
...
void F_displaysimple()
   {
   // open_bool, strat_symbol, tp_price, sl_price, 
   if (PositionSelect(strat_symbol)==true && curr_exposureabs==0) F_currhandling(strat_symbol,0);
   Comment(StringFormat("Open = %G\nOpenNr = %G\nSymbol = %s\nMagic = %G\nTicket = %s\ntp =  %G\ntp2 =  %G\nsl =  %G\nsl2 = %G"+
       "\nCurrencies:         EUR  GBP  AUD  USD  CAD  CHF  JPY"+
       //"\nCurr_order:         %G      %G      %G      %G      %G      %G      %G"+
       "\nCurr_exposure:    %G      %G      %G      %G      %G      %G      %G",
      open_bool,open_nr,strat_symbol,MAGIC_Rf,DoubleToString(OrderTicket(),0),tp_price,tp_price2,sl_price,sl_price2,
      //curr_order[1][1],curr_order[2][2],curr_order[3][3],curr_order[4][4],curr_order[5][5],curr_order[6][6],curr_order[7][7],
      curr_exposure[1][1],curr_exposure[2][2],curr_exposure[3][3],curr_exposure[4][4],curr_exposure[5][5],curr_exposure[6][6],curr_exposure[7][7]
      ));
   }
