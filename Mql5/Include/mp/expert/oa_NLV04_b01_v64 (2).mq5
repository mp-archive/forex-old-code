...
{  double res=0.0,res1=0.0,res2=0.0,res3=0.0,res4=0.0,res5=0.0,tester_tmp=0.0;
   ...
   case 4: {  res=res4;if (AccountEquity()<200) res=0;  }; break;
...
bool F_currallowed(string curr_in,int order_in)
    ...
    */
    ...
    { if (PositionSelect(curr[i3]+curr[j3])==true) { curr_exposure[i3][j3]+=OrderType2();curr_exposure[i3][i3]+=OrderType2(); curr_exposure[j3][j3]-=OrderType2(); }
      Print (curr_in);
      if (curr_in=curr[i3]+curr[j3]); { curr_order[i3][j3]+=order_in;curr_order[i3][i3]+=order_in; curr_order[j3][j3]-=order_in; }  
    }
    for (i3=1;i3<=curr_nr;i3++) if (curr_exposure[i3][i3]==curr_order[i3][i3] && curr_order[i3][i3]!=0) curr_allowed=false;
    return(curr_allowed);            
   }
