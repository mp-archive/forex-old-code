...
int OnInit()
  {  F_SetMQL4PreDefVar(strat_symbol);
      ...
      for (int i=0;i<=strat_nr;i++) MAGIC_Rf_m[i]=StringToInteger(in_MAGIC_Rf)+i;            
  ...
  { if (IsTesting() && testfreq>1) testfrcnt=(testfrcnt+1)%testfreq;
   ...
   if (open_bool==false && b_run_multistrat==false && testfrcnt==0) 
      {  signal1=F_signal(entry_type);  
         ...
         if (OrderLots()>(AccountBalance()-hidden_equity)/in_maxequityperlot) OrderClose2(strat_symbol,NormalizeDouble(OrderLots()-(AccountBalance()-hidden_equity)/in_maxequityperlot,2)); 
         }}
...
{  double res=0.0,res1=0.0,res2=0.0,res3=0.0,res4=0.0,res5=0.0,res6=0.0,res7=0.0,tester_tmp=0.0;
   ...
   if (TesterStatistics(STAT_EQUITY_DDREL_PERCENT)!=0) res7=(TesterStatistics(STAT_PROFIT)/TesterStatistics(STAT_INITIAL_DEPOSIT)+1)/TesterStatistics(STAT_EQUITY_DDREL_PERCENT);
   Print("ULTREC    ",res1);
...
void F_open_now(int order_type,int order_type2)
   {  if (order_type==0) {return;}
      if (TimeCurrent()-lastopen<=10 || OrdersTotal()>0) return;  
...
void F_close(int order_type)
   {  bool b_lezar=false; int b_lezar_kod=0;         
      ...
      if (DayOfWeek()>=5 && Hour()>=closefridaynight && closefridaynight!=0) { b_lezar=true; b_lezar_kod=14;} 
      ...
      if (b_lezar==false) return;
         ...
         if (b_lezar==true)    
         {
           ...
           while (PositionGetDouble(POSITION_VOLUME)>in_maxlot) OrderClose2(strat_symbol,in_maxlot);        
