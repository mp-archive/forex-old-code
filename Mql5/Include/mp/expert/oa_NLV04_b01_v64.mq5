...
//------------------------------------------------------------
   ...
   input int        in_testfreq=1;  int testfrcnt=0; int displfrcn=0;  // to make test faster
      enum teststattype {ULTREC=1,LASTPROFP=2,RDD=3,WITHDRAW=4,NRLOSS=5};
...
//ULTREC,LASTPROFP,RDD
 double OnTester()
{  double res=0.0,res1=0.0,res2=0.0,res3=0.0,res4=0.0,res5=0.0,tester_tmp=0.0;
...
bool F_currallowed(string curr_in,int order_in)
   {
    ...
    { 
      ...
      if (PositionSelect(curr[i3]+curr[j3])==true) { curr_exposure[i3][j3]+=OrderType2();curr_exposure[i3][i3]+=OrderType2(); curr_exposure[j3][j3]-=OrderType2(); }
...
void F_displaysimple()
   ...
   PositionSelect(strat_symbol);
   if (IsTesting()) F_currallowed(strat_symbol,1);
