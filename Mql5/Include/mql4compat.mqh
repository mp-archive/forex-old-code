...
CopyOpen(_Symbol,_Period,0,WHOLE_ARRAY,_Open);
} 

void __Close(double &_Close[]) 
{ 
ArraySetAsSeries(_Close,true);
CopyClose(_Symbol,_Period,0,WHOLE_ARRAY,_Close);
} 

void __High(double &_High[])
{
ArraySetAsSeries(_High,true);
CopyHigh(_Symbol,_Period,0,WHOLE_ARRAY,_High);
}

void __Low(double &_Low[])
{
ArraySetAsSeries(_Low,true);
CopyHigh(_Symbol,_Period,0,WHOLE_ARRAY,_Low);
}

void __Time(datetime &_Time[])
{
ArraySetAsSeries(_Time,true);
CopyTime(_Symbol,_Period,0,WHOLE_ARRAY,_Time);
}

void __Volume(long &_Volume[])
{
ArraySetAsSeries(_Volume,true);
CopyTickVolume(_Symbol,_Period,0,WHOLE_ARRAY,_Volume);
}
...
{
// Unimplemented stub
Print("Error: Not implemented stub.");
return -1;
}
...
int IndicatorCountedMQL4(int &_prev_calculated)
{
...
bool SetIndexBufferMQ4(int index, double &array[])
{
...
{   
   ...
   double Arr[];
   ENUM_TIMEFRAMES timeframe=TFMigrate(tf);
   if(CopyOpen(symbol,timeframe, shift, 1, Arr)>0) 
        return(Arr[0]);
   else return(-1);
}
...
double iClose(string symbol, int tf, int shift)
{
   ...
   double Arr[];
   ENUM_TIMEFRAMES timeframe=TFMigrate(tf);
   if(CopyClose(symbol,timeframe, shift, 1, Arr)>0) 
        return(Arr[0]);
   else return(-1);
}
...
double iHigh(string _symbol,int _timeframe, int _index)
{
   ...
   ArraySetAsSeries(_High,true);
   int _copied=CopyHigh(_symbol,TFMigrate(_timeframe),_index,1,_High);
   if(_copied>0 && _index<_copied) _high=_High[_index];
   return(_high);
}
...
double iLow(string _symbol,int _timeframe,int _index)
{
   double _Low[],_low=0;
   ArraySetAsSeries(_Low,true);
   int _copied=CopyLow(_symbol,TFMigrate(_timeframe),_index,1,_Low);
   if(_copied>0 && _index<_copied) _low=_Low[_index];
   return(_low);
}
...
datetime iTime(string _symbol,int _timeframe,int _index)
{
   ...
   ArraySetAsSeries(_Time,true);
   int _copied=CopyTime(_symbol,TFMigrate(_timeframe),_index,1,_Time);
   if(_copied>0 && _index<_copied) _time=_Time[_index];
   return(_time);
}
...
long iVolume(string _symbol,int _timeframe,int _index)
{
   ...
   ArraySetAsSeries(_Volume,true);
   int _copied=CopyTickVolume(_symbol,TFMigrate(_timeframe),_index,1,_Volume);
   if(_copied>0 && _index<_copied) _volume=_Volume[_index];
   return(_volume);
}
